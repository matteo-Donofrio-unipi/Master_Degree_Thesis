...
def addRowToDB2(FromNodeId, From_author_pub_key, ToNodeId):
    #'FromNodeId', 'From_Author_Pub_Key','ToNodeId', 'To_Author_Pub_key'
    #HERE THE 'ToNodeId' VALUE IS THE ID OF THE PARENT MESSAGE CITED BY THE NEW PUB MSG
...
def computePageRank():
    ...
    if(len(DB_articles_authors) == 0):
        ...
        DB_articles_authors = retrieveAllTangleMessages()
...
def retrieveAllTangleMessages():
    ...
    messages = client.find_messages(indexation_keys=['test_data'])

    print(f'LEN MESSAGES ON TANGLE: {len(messages)}')
    ...
    print(f'Len DB built is: {len(DB_articles_authors)}')
...
def refresh_DBArticlesAuthors():
    ...
    for i in range(len(DB_articles_authors)):
        if(DB_articles_authors.iloc[i]['To_Author_Pub_key']=='Not_available'):
            ToNodeId = DB_articles_authors.iloc[i]['ToNodeId']

            if(ToNodeId in DB_articles_authors['FromNodeId'].values):
                q = DB_articles_authors[DB_articles_authors['FromNodeId']== ToNodeId]['From_Author_Pub_Key']
                To_Author_Pub_key = np.unique(q.values)[0]
                #print(To_Author_Seed)
                DB_articles_authors.iloc[i]['To_Author_Pub_key'] = str(To_Author_Pub_key)
    DB_articles_authors.to_csv('./DB_articles_authors_built.csv',index = False)

#write a new msg id to the DB file

def refresh_DBArticlesAuthors2():
    #'FromNodeId', 'From_Author_Pub_Key','ToNodeId', 'To_Author_Pub_key'
    global DB_articles_authors

    for i in range(len(DB_articles_authors)):

        if(DB_articles_authors.loc[i]['ToNodeId'] in DB_articles_authors['FromNodeId'].values):
            ToNodeId = DB_articles_authors.loc[i]['ToNodeId']
           
            To_Author_Pub_key = DB_articles_authors[DB_articles_authors['FromNodeId']== ToNodeId]['From_Author_Pub_Key']

            To_Author_Pub_key = np.unique(To_Author_Pub_key.values)[0]

            DB_articles_authors.iloc[i]['To_Author_Pub_key'] = str(To_Author_Pub_key)
        else:
            DB_articles_authors.iloc[i]['To_Author_Pub_key'] = 'Not_available'

    DB_articles_authors.to_csv('./DB_articles_authors_built2.csv',index = False)
